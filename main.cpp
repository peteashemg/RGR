...
vector<unsigned char> processDLL(const string& dllName, const vector<unsigned char>& data, bool encryptMode, const string& key){
    ...
    if (!cipher){
        throw runtime_error("Не удалось загрузить DLL");
    }
...
vector<unsigned char> processHill(
    ...
    return processDLL("HillCipher.dll", data, encryptMode, keyString);
}

vector<unsigned char> processShamir(
    const vector<unsigned char>& data,
    bool encryptMode){
    int p;
    ...
    if (encryptMode) {
        ...
        key = to_string(p) + " " + to_string(ca) + " " + to_string(da) + " " + to_string(cb);
    }
    else{
        int db;
        cout << "Введите обратный ключ Боба dB: ";
        cin >> db;
        key = to_string(p) + " " + to_string(db);
    }

    return processDLL("ShamirCipher.dll", data, encryptMode,  key);
}

vector<unsigned char> processElGamal(
    const vector<unsigned char>& data,
    bool encryptMode)
{
    int p;
    ...
    if (encryptMode){
        ...
        cin >> x;
        key = to_string(p) + " " + to_string(x);
    }

    return processDLL("ElGamalCipher.dll", data, encryptMode, key);
}
